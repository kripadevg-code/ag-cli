...
class SettingsController extends GetxController with ScrollMixin {
  static SettingsController get find => Get.find();

  final SettingsRepo repo;
  SettingsController({required this.repo});

  static const String historyKey = 'settings_search';
  ...
  List<Map<String, dynamic>>? _buildFilter() {
    if (_isSearchFieldVisible && searchFieldController.text.trim().isNotEmpty) {
      // TODO: return QueryHelper.getSettingsSearchFilterQuery(searchFieldController.text.trim());
    }
    if (_selectedFilter.value?.id == 0) return [];
    // TODO: return QueryHelper.getSettingsStateFilterQuery(_selectedFilter.value?.name);
    return [];
  }
