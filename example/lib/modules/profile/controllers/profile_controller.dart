...
class ProfileController extends GetxController with ScrollMixin {
  ...
  static const String historyKey = 'profile_search';
  ...
  // final List<ProfileModel> _items = [];
  ...
  List<Map<String, dynamic>>? _buildFilter() {
    if (_isSearchFieldVisible && searchFieldController.text.trim().isNotEmpty) {
      // TODO: return QueryHelper.getProfileSearchFilterQuery(searchFieldController.text.trim());
    }
    if (_selectedFilter.value?.id == 0) return [];
    // TODO: return QueryHelper.getProfileStateFilterQuery(_selectedFilter.value?.name);
    return [];
  }
