...
class HomeController extends GetxController with ScrollMixin {
  ...
  static const String historyKey = 'home_search';
  ...
  List<Map<String, dynamic>>? _buildFilter() {
    if (_isSearchFieldVisible && searchFieldController.text.trim().isNotEmpty) {
      // TODO: return QueryHelper.getHomeSearchFilterQuery(searchFieldController.text.trim());
    }
    if (_selectedFilter.value?.id == 0) return [];
    // TODO: return QueryHelper.getHomeStateFilterQuery(_selectedFilter.value?.name);
    return [];
  }
