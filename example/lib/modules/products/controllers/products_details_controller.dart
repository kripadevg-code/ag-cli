...
class ProductsDetailsController extends GetxController with ScrollMixin {
  static ProductsDetailsController get find => Get.find();

  final ProductsDetailsRepo repo;
  ProductsDetailsController({required this.repo});

  static const String historyKey = 'products_details_search';
  ...
  // final List<ProductsDetailsModel> _items = [];
  ...
  List<Map<String, dynamic>>? _buildFilter() {
    if (_isSearchFieldVisible && searchFieldController.text.trim().isNotEmpty) {
      // TODO: return QueryHelper.getProductsDetailsSearchFilterQuery(searchFieldController.text.trim());
    }
    if (_selectedFilter.value?.id == 0) return [];
    // TODO: return QueryHelper.getProductsDetailsStateFilterQuery(_selectedFilter.value?.name);
    return [];
  }
