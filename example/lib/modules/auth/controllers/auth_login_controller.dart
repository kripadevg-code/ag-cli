...
class AuthLoginController extends GetxController with ScrollMixin {
  static AuthLoginController get find => Get.find();

  final AuthLoginRepo repo;
  AuthLoginController({required this.repo});

  static const String historyKey = 'auth_login_search';
  ...
  // final List<AuthLoginModel> _items = [];
  ...
  List<Map<String, dynamic>>? _buildFilter() {
    if (_isSearchFieldVisible && searchFieldController.text.trim().isNotEmpty) {
      // TODO: return QueryHelper.getAuthLoginSearchFilterQuery(searchFieldController.text.trim());
    }
    if (_selectedFilter.value?.id == 0) return [];
    // TODO: return QueryHelper.getAuthLoginStateFilterQuery(_selectedFilter.value?.name);
    return [];
  }
