...
import 'package:example/modules/orders/components/orders_card.dart';
...
import 'package:example/modules/orders/components/orders_app_bar.dart';
import 'package:example/modules/orders/components/orders_appbar_filter.dart';
...
class OrdersPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => PopScope(
    onPopInvokedWithResult: (_, _) => OrdersController.find.onBackPressed(),
    child: Scaffold(
              ...
              if (controller.isSearchFieldVisible)
                Positioned(
                  ...
                  child: SearchHistoryPanel(
                    historyKey: OrdersController.historyKey,
...
class _OrdersCardList extends StatelessWidget {
  const _OrdersCardList({required this.controller});
  final OrdersController controller;

  @override
  Widget build(BuildContext context) => SliverList(
    // TODO: replace childCount with controller.items.length
    delegate: SliverChildBuilderDelegate((_, i) => const OrdersCard(), childCount: 0),
  );
}
